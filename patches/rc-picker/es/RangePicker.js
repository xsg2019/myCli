...
function InnerRangePicker(props) {
  ...
  function triggerChange(newValue, sourceIndex) {
    ...
    if (startValue && endValue) {
      ...
      picker !== 'week' && picker !== 'quarter' && picker !== 'time' && !isSameDate(generateConfig, startValue, endValue)) {
        if (startValue > endValue) {
          values = [startValue, startValue];
          setViewDate(startValue, 1);
        } else {
    ...
    if (onCalendarChange) {
      ...
      if (sourceIndex === 1 || sourceIndex === null) {
        ...
        onCalendarChange(values, [startStr, endStr], info, isLabel);
      }
    } // >>>>> Trigger `onChange` event


    var canStartValueTrigger = canValueTrigger(startValue, 0, mergedDisabled, allowEmpty);
    var canEndValueTrigger = canValueTrigger(startValue > endValue ? startValue : endValue, 1, mergedDisabled, allowEmpty);
  ...
  var rangeList = rangeLabels.map(function (label) {
    ...
    return {
      ...
      onClick: function onClick() {
        // 点击 label 触发更新
        triggerChange(newValues, null, true);
  ...
  function renderPanels() {
    ...
    if (picker !== 'time' && !showTime) {
      ...
      var showDoublePanel = currentMode === picker; // 左侧时间面板
      ...
      // viewDate nextViewDate 为初始input中的值
